...
stat_type 값 & region 규칙:
  ...
  '화재장소(시도)'  → region=시도명 × 장소 교차 집계 (지역별 건물유형 통계)
                     "서울 아파트", "경기 단독주택" 같은 지역+건물 조합 질문에 사용

시도명 (17개): '서울특별시','부산광역시','대구광역시','인천광역시','광주광역시','대전광역시','울산광역시','세종특별자치시','경기도','강원도','충청북도','충청남도','전라북도','전라남도','경상북도','경상남도','제주특별자치도'
  -- 세종특별자치시는 2012-07-01 이후 데이터부터 포함
...
- 전국 합계: stat_type='시도별' + region 조건 없이 → 17개 시도 합산 = 전국 합계
...
[건물유형별 통계 필터 - 두 가지 접근]
① 구체적 건물명 → stat_type='화재장소(전국)' (전국) 또는 '화재장소(시도)' (지역별)
  category_main에 정확한 건물명 사용 (대표값):
    아파트='아파트', 주상복합='주상복합아파트', 단독주택='단독주택',
    다세대='다세대주택', 연립='연립주택', 다가구='다가구주택',
    창고='창고, 물품저장소', 냉동창고='냉장, 냉동창고',
    병원='병원', 종합병원='종합병원', 요양병원='요양병원', 의원='의원',
    호텔='호텔', 모텔='모텔', 여관='여관',
    음식점='한식'(또는 양식,중식 등), 할인점='할인점(마트)',
    승용차='승용자동차', 화물차='화물자동차'
② 포괄적 분류 → stat_type='발화장소', category_main+category_sub 조합
  주거 전체: category_main='주거' (공동주택+단독주택 합산)
  공동주택: category_main='주거', category_sub='공동주택'
  공장:     category_main='산업시설', category_sub='공장시설'
  창고:     category_main='산업시설', category_sub='창고시설'
  음식점:   category_main='생활서비스', category_sub='음식점'
"공장" 질문은 ②를 우선 사용 (공장 관련 category_main이 다수로 세분화됨)
"아파트" 질문은 ①을 사용 (category_main='아파트' 존재)
[중요] stat_type='시도별'에는 category_main/category_sub 컬럼이 없음 → 건물유형 필터 시 절대 시도별 사용 금지
지역+건물 조합: stat_type='화재장소(시도)', region=시도명, category_main=건물명 (예: 경기도+아파트 → 화재장소(시도)+경기도+아파트)
...
- 건물유형별 연도 합산: SELECT YEAR(fire_date) AS 연도, SUM(fire_count) AS 화재건수, SUM(death_count) AS 사망자, SUM(damage_amount) AS 재산피해 FROM vfs_data_api WHERE stat_type='화재장소(전국)' AND category_main='아파트' GROUP BY YEAR(fire_date) ORDER BY 연도 DESC
...
컬럼:
  ...
  -- 주의: fire_date 컬럼 없음. 날짜 필터는 반드시 year=, month=, day= 사용
...
[지역 값]
  ...
  '경기도','강원도','충청북도','충청남도','전북특별자치도','전라남도','경상북도','경상남도','제주특별자치도'
  -- vfs_data_csv는 전라북도 → 전북특별자치도로 저장됨
...
export async function generateSQL(question, intent, entities) {
  ...
  // vfs_data_api에 district 컬럼 없음 → stats 쿼리에서 district 힌트 제외
  if (entities.district && intent !== "stats") hints.push(`시군구: "${entities.district}"`);
  if (entities.district && intent === "stats") hints.push(`시군구 "${entities.district}"는 vfs_data_api에 없음 → 해당 시군구 속한 시도(${entities.region || "해당 시도"})로만 필터하세요`);
  if (entities.year && intent === "stats") {
    if (!entities.building) {
      hints.push(`연도: ${entities.year} → 반드시 stat_type='시도별' 포함. 전국 합산: WHERE stat_type='시도별' AND YEAR(fire_date)=${entities.year}. 월별 요약 시: GROUP BY MONTH(fire_date). 연도 합산 시: SUM() 집계 (LIMIT 없이)`);
    }
    // 건물유형 있으면 year hint는 날짜 조건만 추가 (stat_type은 건물유형 힌트가 결정)
    // → 아래 building 힌트 블록에서 통합 처리
  }
  if (entities.year && intent === "case")  hints.push(`연도: ${entities.year}`);
  if (entities.building && intent === "stats") {
    const b = entities.building;
    // stat_type='화재장소(전국/시도)' 매핑: region 있으면 화재장소(시도)로 전환 가능
    // stat_type='발화장소' 매핑: 전국 전용 (region 있어도 전국 데이터만 제공)
    const BUILDING_STAT_MAP = [
      // [패턴, 전국 stat_type+category, 지역별 가능 여부]
      [/아파트|공동주택/,     "stat_type='화재장소(전국)', category_main='아파트'",          true],
      [/주상복합/,            "stat_type='화재장소(전국)', category_main='주상복합아파트'",   true],
      [/단독주택/,            "stat_type='화재장소(전국)', category_main='단독주택'",         true],
      [/다세대|빌라/,         "stat_type='화재장소(전국)', category_main='다세대주택'",        true],
      [/연립/,                "stat_type='화재장소(전국)', category_main='연립주택'",          true],
      [/다가구/,              "stat_type='화재장소(전국)', category_main='다가구주택'",         true],
      [/호텔|모텔|여관/,      "stat_type='화재장소(전국)', category_main='호텔'",              true],
      [/자동차|승용차|차량/,  "stat_type='화재장소(전국)', category_main='승용자동차'",        true],
      [/화물차|트럭/,         "stat_type='화재장소(전국)', category_main='화물자동차'",        true],
      [/주거|주택/,           "stat_type='발화장소', category_main='주거'",                   false],
      [/공장|제조/,           "stat_type='발화장소', category_main='산업시설', category_sub='공장시설'", false],
      [/창고|물류/,           "stat_type='발화장소', category_main='산업시설', category_sub='창고시설'", false],
      [/음식점|식당/,         "stat_type='발화장소', category_main='생활서비스', category_sub='음식점'", false],
      [/숙박/,                "stat_type='발화장소', category_main='판매,업무시설', category_sub='숙박시설'", false],
      [/병원|의원|의료/,      "stat_type='발화장소', category_main='의료,복지시설', category_sub='의료시설'", false],
      [/학교|교육/,           "stat_type='발화장소', category_main='교육시설'",               false],
      [/마트|백화점|판매/,    "stat_type='발화장소', category_main='판매,업무시설', category_sub='판매시설'", false],
    ];
    const matchedEntry = BUILDING_STAT_MAP.find(([re]) => re.test(b));
    if (matchedEntry) {
      const [, baseStat, regionalCapable] = matchedEntry;
      // region 있고 지역별 조회 가능한 타입 → 화재장소(시도)로 전환
      if (entities.region && regionalCapable) {
        const regional = baseStat.replace("화재장소(전국)", "화재장소(시도)");
        hints.push(`건물/장소 유형: "${b}" + 지역: "${entities.region}" → 반드시 ${regional}, region='${entities.region}' 사용`);
      } else if (entities.region && !regionalCapable) {
        hints.push(`건물/장소 유형: "${b}" → 반드시 ${baseStat} 사용 (이 분류는 지역별 데이터 없음, 전국 기준만 제공)`);
      } else {
        hints.push(`건물/장소 유형: "${b}" → 반드시 ${baseStat} 사용`);
      }
      // year hint 통합: 날짜 조건만 추가
      if (entities.year) {
        hints.push(`연도: ${entities.year} → YEAR(fire_date)=${entities.year} 조건 추가. SUM() 집계 (LIMIT 없이)`);
      }
    } else {
      // 매핑 없는 경우
      if (entities.region) {
        hints.push(`건물/장소 유형: "${b}" + 지역: "${entities.region}" → stat_type='화재장소(시도)', region='${entities.region}', category_main=실제 DB 값 선택`);
      } else {
        hints.push(`건물/장소 유형: "${b}" → stat_type='화재장소(전국)' (전국) 또는 '화재장소(시도)' (지역별), category_main은 실제 DB 값과 가장 유사한 것 선택`);
      }
      if (entities.year) hints.push(`연도: ${entities.year} → YEAR(fire_date)=${entities.year} 조건 추가`);
    }
  } else if (entities.building) {
    hints.push(`건물/장소 유형: "${entities.building}"`);
  }
...
function validateSQL(sql) {
  ...
  // DML/DDL 차단
  if (/\b(DROP|DELETE|UPDATE|INSERT|ALTER|TRUNCATE|CREATE|EXEC|REPLACE|RENAME|CALL)\b/i.test(clean)) {
  ...
  if (/\bINTO\s+(OUTFILE|DUMPFILE)\b/i.test(clean)) throw new Error("허용되지 않은 접근입니다");
  if (/\bINFORMATION_SCHEMA\b/i.test(clean)) throw new Error("허용되지 않은 접근입니다");
  if (/@@\w+/.test(clean)) throw new Error("허용되지 않은 접근입니다");
  // 중간 세미콜론(스택 쿼리) 차단 (끝 세미콜론은 이미 제거됨)
  if (/;/.test(clean)) throw new Error("허용되지 않은 접근입니다");
  // 허용 테이블 외 접근 차단
  ...
  if (!isCaseTbl) {
    ...
    if (stMatch) {
      ...
      // 시도별: category 필터 무의미 → 제거
      if (stMatch[1] === "시도별") {
        if (/\bcategory_main\b/i.test(clean) || /\bcategory_sub\b/i.test(clean)) {
          clean = clean.replace(/\bAND\s+category_main\s*=\s*'[^']*'/gi, "")
                       .replace(/\bcategory_main\s*=\s*'[^']*'\s+AND\b/gi, "")
                       .replace(/\bAND\s+category_sub\s*=\s*'[^']*'/gi, "")
                       .replace(/\bcategory_sub\s*=\s*'[^']*'\s+AND\b/gi, "");
          nationalNote += `[안내: 시도별 통계는 건물/장소 분류가 없습니다. 건물유형 필터를 제거하고 지역별 전체 피해 집계로 답변합니다]\n`;
        }
      }
      ...
      // 화재장소(시도): region='전국'은 존재하지 않으므로 region 조건 제거 (전체 시도 반환)
      else if (stMatch[1] === "화재장소(시도)" && regionMatch && regionMatch[1] === "전국") {
        clean = clean.replace(/\bAND\s+region\s*=\s*'전국'/gi, "")
                     .replace(/\bregion\s*=\s*'전국'\s+AND\b/gi, "")
                     .replace(/\bWHERE\s+region\s*=\s*'전국'\b/gi, "WHERE 1=1")
                     .trim();
  ...
  // vfs_data_csv: fire_date 컬럼 없음 → YEAR/MONTH/DAY(fire_date) 변환
  if (isCaseTbl) {
    clean = clean.replace(/\bYEAR\s*\(\s*fire_date\s*\)\s*=\s*(\d{4})/gi,  "year=$1");
    clean = clean.replace(/\bMONTH\s*\(\s*fire_date\s*\)\s*=\s*(\d{1,2})/gi, "month=$1");
    clean = clean.replace(/\bDAY\s*\(\s*fire_date\s*\)\s*=\s*(\d{1,2})/gi,  "day=$1");
    // 남은 fire_date 단독 참조 → year로 대체 (오류 방지)
    clean = clean.replace(/\bfire_date\b/gi, "year");
  }

  // vfs_data_csv: 전북특별자치도 명칭 교정 (= 및 LIKE 모두)
  if (isCaseTbl) {
    clean = clean.replace(/region\s*(?:=|LIKE)\s*'전라북도[^']*'/g, "region='전북특별자치도'");
  }
  ...
  // vfs_data_csv: year 범위 교정 (2019~2023만 존재)
  if (isCaseTbl) {
    // year = YYYY (> 2023이면 제거)
    const yearEq = clean.match(/\byear\s*=\s*(\d{4})\b/i);
    if (yearEq && Number(yearEq[1]) > 2023) {
      clean = clean.replace(/\bAND\s+year\s*=\s*\d{4}\b/gi, "")
                   .replace(/\byear\s*=\s*\d{4}\s+AND\b/gi, "")
                   .replace(/\bWHERE\s+year\s*=\s*\d{4}\b/gi, "WHERE 1=1")
                   .trim();
    }
    // year > 2023 or year >= 2024 → 제거
    clean = clean.replace(/\bAND\s+year\s*>=?\s*(?:202[4-9]|20[3-9]\d|\d{5,})\b/gi, "");
    clean = clean.replace(/\byear\s*>=?\s*(?:202[4-9]|20[3-9]\d|\d{5,})\b\s+AND\b/gi, "");
    // year BETWEEN s AND e (e > 2023이면 2023으로 클램프)
    clean = clean.replace(/\byear\s+BETWEEN\s+(\d{4})\s+AND\s+(\d{4})/gi, (_, s, e) => {
      const startY = Math.min(Number(s), 2023);
      const endY   = Math.min(Number(e), 2023);
      if (startY > 2023) return "1=1";
      return `year BETWEEN ${startY} AND ${endY}`;
    });
  }

  // vfs_data_api: district 컬럼 없음 → 제거
  if (!isCaseTbl) {
    clean = clean.replace(/\bAND\s+district\s*(?:=|LIKE)\s*'[^']*'/gi, "");
    clean = clean.replace(/\bdistrict\s*(?:=|LIKE)\s*'[^']*'\s+AND\b/gi, "");
    clean = clean.replace(/\bWHERE\s+district\s*(?:=|LIKE)\s*'[^']*'\b/gi, "WHERE 1=1");
  }
  ...
  if (!hasAgg && !/\bLIMIT\b/i.test(clean)) {
    ...
    clean = clean + ` LIMIT ${lim}`;
  }

  // LIMIT 상한 캡 (과도한 조회 방지)
  const MAX_LIMIT = isCaseTbl ? 20 : 200;
  clean = clean.replace(/\bLIMIT\s+(\d+)\b/gi, (_, n) => `LIMIT ${Math.min(Number(n), MAX_LIMIT)}`);

  const isLimited = /\bLIMIT\b/i.test(clean);
  return { sql: clean, note: nationalNote, isLimited };
}
...
function formatCases(rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (r.device_main && r.device_main !== "미상") {
      const dev = [r.device_main, r.device_sub].filter((v) => v && v !== "미상").join(" > ");
      lines.push(`- 발화기기: ${dev}`);
    }
    ...
    const weatherParts = [];
    if (r.temperature != null) weatherParts.push(`기온 ${r.temperature}°C`);
    if (r.humidity != null) weatherParts.push(`습도 ${r.humidity}%`);
    if (r.wind_speed && r.wind_speed !== "미상") weatherParts.push(`풍속 ${r.wind_speed}`);
    if (weatherParts.length > 0) lines.push(`- 기상: ${weatherParts.join(", ")}`);
  }
  return lines.join("\n");
}
...
async function buildSQLContext(rows, sql, intent, entities, extraNote, isLimited) {
  ...
  if (entities.building) condParts.push(entities.building);
